...
const Konta = () => {
  ...
  const handleDeleteAccount = async (accountId) => {
    try {
      ...
      toast.error(error?.response?.data?.message || "Wystąpił błąd podczas usuwania konta.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className='w-full py-4'>
        ...
        {data?.length === 0 ? (
          ...
          <div className='w-full max-w-screen-lg mx-auto grid grid-cols-1 py-4 gap-3'>
            {data?.map((acc, index) => (
              <div key={index} className='w-full h-24 flex items-center gap-4 bg-gray-50 p-3 rounded shadow'>
                <div className='flex items-center gap-3'>
                  <div>{ICONS[acc?.account_name?.toLowerCase()]}</div>
                  <div className='space-y-1'>
                    <p className='text-black text-lg font-bold'>{acc?.account_name}</p>
                    <span className='text-gray-600 text-sm font-light'>{acc?.account_number}</span>
                  </div>
                </div>
